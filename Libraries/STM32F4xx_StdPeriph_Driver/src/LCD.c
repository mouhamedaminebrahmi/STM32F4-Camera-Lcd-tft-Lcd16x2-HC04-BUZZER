...
static void LCD_FSMCConfig(void)
{
	...
	/* FSMC????? */
	FSMC_NORSRAMTimingInitStructure.FSMC_AddressSetupTime = 20;//1;   /* ??????  */
	FSMC_NORSRAMTimingInitStructure.FSMC_AddressHoldTime = 0;	   
	FSMC_NORSRAMTimingInitStructure.FSMC_DataSetupTime = 20;//1;	   /* ??????  */
...
{
	LCD_PutCharFont(Xpos, Ypos, ASCII, charColor, bkColor, SYSTEM_8x16);
}
void GUI_Text1(uint16_t Xpos, uint16_t Ypos, uint8_t *str,uint16_t Color, uint16_t bkColor)
{
    uint8_t TempChar;
    do
    {
...
{
	LCD_CleanTextFont(Xpos, Ypos, str, Color, SYSTEM_8x16);
